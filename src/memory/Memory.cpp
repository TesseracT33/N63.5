...
namespace Memory
{
	...
	T ReadPhysical(const size_t number_of_bytes, const u32 physical_address)
	{
		...
		//const auto read_fun = [&] {
		//	     if constexpr (sizeof T == 1) return read_table_8[page];
		//	else if constexpr (sizeof T == 2) return read_table_16[page];
		//	else if constexpr (sizeof T == 4) return read_table_32[page];
		//	else if constexpr (sizeof T == 8) return read_table_64[page];
		//	else                              static_assert(false);
		//}();

		//const auto read_value = std::invoke(read_fun, physical_address);
		///* Reverse the read bytes if the host and n64 endianness are different. */
		//const T ret_value = convert_endian<T>(read_value);
		//return ret_value;
		return 0;
	}
	...
	{
		const u32 page = physical_address >> 20;

		//const auto write_fun = [&] {
		//	if constexpr (sizeof T == 1) return write_table_8[page];
		//	else if constexpr (sizeof T == 2) return write_table_16[page];
		//	else if constexpr (sizeof T == 4) return write_table_32[page];
		//	else if constexpr (sizeof T == 8) return write_table_64[page];
		//	else                              static_assert(false);
		//}();

		//const T write_value = convert_endian<T>(data);
		//std::invoke(write_fun, physical_address, write_value);
	}


	template u8  ReadPhysical<u8>(const size_t number_of_bytes, const u32 addr);
	template s8  ReadPhysical<s8>(const size_t number_of_bytes, const u32 addr);
	template u16 ReadPhysical<u16>(const size_t number_of_bytes, const u32 addr);
	template s16 ReadPhysical<s16>(const size_t number_of_bytes, const u32 addr);
	template u32 ReadPhysical<u32>(const size_t number_of_bytes, const u32 addr);
	template s32 ReadPhysical<s32>(const size_t number_of_bytes, const u32 addr);
	template u64 ReadPhysical<u64>(const size_t number_of_bytes, const u32 addr);
	template s64 ReadPhysical<s64>(const size_t number_of_bytes, const u32 addr);

	template void WritePhysical<u8>(const size_t number_of_bytes, const u32 addr, const u8 data);
	template void WritePhysical<s8>(const size_t number_of_bytes, const u32 addr, const s8 data);
	template void WritePhysical<u16>(const size_t number_of_bytes, const u32 addr, const u16 data);
	template void WritePhysical<s16>(const size_t number_of_bytes, const u32 addr, const s16 data);
	template void WritePhysical<u32>(const size_t number_of_bytes, const u32 addr, const u32 data);
	template void WritePhysical<s32>(const size_t number_of_bytes, const u32 addr, const s32 data);
	template void WritePhysical<u64>(const size_t number_of_bytes, const u32 addr, const u64 data);
	template void WritePhysical<s64>(const size_t number_of_bytes, const u32 addr, const s64 data);
}