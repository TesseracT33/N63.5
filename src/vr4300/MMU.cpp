...
namespace VR4300
{
	...
	void SetActiveVirtualToPhysicalFunctions()
	{
		...
		if (cop0_reg.status.ksu == 0 || cop0_reg.status.erl == 1 || cop0_reg.status.exl == 1) { /* Kernel mode */
		...
		else if (cop0_reg.status.ksu == 1) { /* Supervisor mode */
		...
		else if (cop0_reg.status.ksu == 2) { /* User mode */
			operating_mode = User;
	...
	u32 VirtualToPhysicalAddressKernelMode32(u64 virt_addr, bool& cacheable_area)
	{
		if (virt_addr < 0xFFFF'FFFF'0000'0000) { /* TODO: same for user and supervisor modes? */
			...
			cacheable_area = false;
			return VirtualToPhysicalAddressTlb<operation>(virt_addr); /* TLB mapped */
		}
	}
